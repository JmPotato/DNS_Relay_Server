...
import socket   #套接字，用于和远程DNS服务器通信
...
class DNSResolver():
    ...
    def __init__(self, request_data, local_file='dnsrelay.txt', remote_server='223.5.5.5'):  #阿里公用DNS服务器
        ...
        self.request = {
            'data': request_data,                               #原始查询报文
            'flags': self.parseFlags(request_data),             #解析报头中的flag各字段，存储在flags字典
            'header': self.parseDNSHeader(request_data),        #解析整个报头，报各字段转化为整数，存储在header字典
            'question': self.parseDNSQuestion(request_data)     #解析问题区域，获取要查询的域名，查询类型，存储在question字典
        }

        # 填写响应报文
        self.response_data = self.queryIntegratedServer(        #综合查询函数，返回响应报文
            local_file, remote_server)
        self.response = {
            'data': self.response_data,                              #原始响应报文
            'flags': self.parseFlags(self.response_data),            #解析报头中的flag各字段，存储在flags字典
            'header': self.parseDNSHeader(self.response_data),       #解析整个报头，报各字段转化为整数，存储在header字典
            'question': self.parseDNSQuestion(self.response_data),   #解析QUESTION问题区域，获取要查询的域名，查询类型，存储在question字典
            'answer': self.parseDNSAnswer(self.response_data)        #解析ANSWER回答区域，获取各字段值，其中包括要查询得到的IP地址，存储在answer字典中
        }

    # # 翻译状态码，用于在控制台输出报文中的flag中各字段信息
    ...
    def parseDNSHeader(self, bytes_data):
        try:
            (header_id, header_flags, header_qdcount, header_ancount, header_nscount,
             header_arcount) = struct.unpack('>HHHHHH', bytes_data[0:12])               #将报头的前12个字节，每两个字节一组变为2字节无符号整数
        ...
        try:
            ...
            while(count <= length):
                ...
                if(count > length and bytes_data[index+count] != 0):
                    ...
                    index = index + count
                    count = 1
            (question_qtype, question_qclass) = struct.unpack(
                '>HH', bytes_data[index+count+1:index+count+1+4])
    ...
    def parseDNSAnswer(self, bytes_data):
        try:
            ...
            elif self.request['question']['QTYPE'] == 28:
                (answer_qtype, answer_qclass, answer_ttl, answer_rdlength,
                    answer_data_1, answer_data_2, answer_data_3, answer_data_4, answer_data_5, answer_data_6, answer_data_7, answer_data_8) = struct.unpack('>HHLHHHHHHHHH', bytes_data[-26:])
                return dict(ANAME=49164, ATYPE=answer_qtype, ACLASS=answer_qclass, ATTL=answer_ttl, ARDLENGTH=answer_rdlength,
                            ARDATA=str(hex(answer_data_1))[2:] + ':' + str(hex(answer_data_2))[2:] + ':' + str(hex(answer_data_3))[2:] + ':' + str(hex(answer_data_4))[2:] + ':' + str(hex(answer_data_5))[2:] + ':' + str(hex(answer_data_6))[2:] + ':' + str(hex(answer_data_7))[2:] + ':' + str(hex(answer_data_8))[2:])
        except:
            return dict(ANAME=0, ATYPE=0, ACLASS=0, ATTL=0, ARDLENGTH=0, ARDATA='0.0.0.0')
    ...
    def queryLocalServer(self, local_file):
        ...
        #构造回传的响应报文
        response_data = struct.pack('>HHHHHH', self.request['header']['ID'], flags, self.request['header']
                                    ['QDCOUNT'], answer, self.request['header']['NSCOUNT'], self.request['header']['ARCOUNT'])
        ...
        if answer != 0:
            ...
            response_data += struct.pack('BBBB',
                                         int(ip[0]), int(ip[1]), int(ip[2]), int(ip[3]))  #加上Answer回答区域最后的资源数据（即IP地址）字段
    ...
    def queryIntegratedServer(self, local_file, remote_server):
        ...
        if(self.ip_result == ''):
            response_data = self.queryRemoteServer(
                remote_server)                                      # 本地未查询到记录后转发远程服务器查询
